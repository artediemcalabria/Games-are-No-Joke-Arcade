  ...
  const askCoach = async (overrideQuestion?: string) => {
    ...
    try {
      const result = endpoint
        ? await askBackendCoach(endpoint, selectedMode, question, prototype, gameTakeaways, gameNotes)
        : buildFallbackAnswer(selectedMode.id, question, prototype);
      ...
      setStatus(endpoint ? 'Answer generated by Gemini coach.' : 'Offline template used. Add VITE_AI_COACH_ENDPOINT to enable Gemini.');
    } catch (caught) {
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="pb-10 max-w-6xl mx-auto">
      ...
      <section className="grid grid-cols-1 xl:grid-cols-[1fr_340px] gap-4">
        <div className="space-y-4">
          ...
          <section className="bg-black/60 border border-white/10 rounded-xl p-5">
            ...
            <div className="mt-4 space-y-3">
              ...
              {coachHistory.slice(0, 5).map((session) => (
                <details key={session.id} className="rounded-lg border border-white/10 bg-black/45 p-3">
          ...
          <div className="bg-black/60 border border-white/10 rounded-xl p-5">
            ...
            <div className="mt-4 space-y-3">
              ...
              {coachNotes.slice(0, 5).map((note) => (
                <div key={note.id} className="rounded-lg border border-white/10 bg-black/45 p-3">
...
function PrototypeSnapshot({ completedCount, missingSteps, prototypeContext }: { completedCount: number; missingSteps: string[]; prototypeContext: string }) {
  return (
    <div className="arcade-border-pink glass-panel-pink rounded-xl p-5">
      ...
      <div className="mt-4 rounded-lg border border-white/10 bg-black/45 p-3">
      ...
      <details className="mt-3 rounded-lg border border-white/10 bg-black/45 p-3">
...
async function askBackendCoach(
  ...
  if (!response.ok) {
    ...
    throw new Error(text || `Coach backend returned ${response.status}.`);
  }
...
function buildFallbackAnswer(mode: CoachModeId, question: string, prototype: Record<string, string>) {
  const topic = prototype.topic?.trim() || 'your topic';
  const mechanic = prototype.coreMechanic?.trim();
  const learningGoal = prototype.learningGoal?.trim();
  const rules = prototype.rules?.trim();
