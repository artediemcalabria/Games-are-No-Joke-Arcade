...
export default function GeminiCoach() {
  ...
  const askCoach = async (overrideQuestion?: string) => {
    ...
    try {
	      ...
	      const finalAnswer = ensureAnswerShape(result.answer);
      ...
      saveCoachSession({
        ...
        createdAt: new Date().toISOString(),
	      ...
	      const modelLabel = result.model ? ` Gemini model used: ${result.model}${result.fallbackUsed ? ' (fallback).' : '.'}` : '';
	      setStatus(`Answer generated by Gemini coach.${modelLabel}`);
    } catch (caught) {
      const fallback = ensureAnswerShape(buildFallbackAnswer(selectedMode.id, question, prototype));
...
async function askBackendCoach(
  ...
  return {
    answer: data.answer || 'The coach returned an empty answer. Try a more specific question.',
    model: data.model,
